  ...
  const handleSubmit = async (event) => {
    try {
      ...
      if (!entityResponse || !entityResponse.success) {
        emitEvent({ action: 'veripass-quick-user-kyc::error', error: entityResponse, eventHandler: onEvent });
        return null;
      }

      // Update parent states
      emitEvent({ action: 'veripass-quick-user-kyc::updated', payload: entityResponse, eventHandler: onEvent });
    } catch (error) {
      console.error(error);
      setIsLoading(false);

      emitEvent({ action: 'veripass-quick-user-kyc::error', error, eventHandler: onEvent });
    }
  };
